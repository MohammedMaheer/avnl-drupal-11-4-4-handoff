/**
 * @file
 * Provides UI/UX progressive enhancements on avnl's theme settings by
...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.avnl.colorSchemes;
  ...
  function synchronizeInputs(changedInput, inputToSync) {
    ...
    changedInput.setAttribute('data-avnl-custom-color', changedInput.value);
    inputToSync.setAttribute('data-avnl-custom-color', changedInput.value);
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-avnl-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute('data-avnl-custom-color');
        });
    }
  }
  ...
  /**
   * Initializes avnl theme-settings color picker.
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-avnl-custom-color',
      textInput.getAttribute('data-avnl-custom-color'),
    );
  ...
  /**
   * avnl Color Picker behavior.
   *
   * @type {Drupal~behavior}
   * @prop {Drupal~behaviorAttach} attach
   *   Initializes color picker fields.
   */
  Drupal.behaviors.avnlColorPicker = {
    attach: () => {
      ...
      const colorTextInputs = once(
        'avnl-color-picker',
        '[data-drupal-selector="avnl-color-picker"] input[type="text"]',
      );
